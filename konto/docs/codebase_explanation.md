...
7. **DX Client Generator Phase 4**: Designed and implemented the `@konto/cli` generator. It dynamically emits strict application client wrappers into `node_modules/.konto`, imports the root `konto.config.ts`, and bridges Zod schemas into both runtime `.parse()` validation and generated TypeScript metadata inference.
...
9. **Zod Runtime Validation (Phase 3 Exploit)**: Dropped the string DSL and fully integrated Zod schemas into `konto.config.ts`. The generated `.konto` client uses `jiti` to dynamically load these schemas at runtime and executes `.parse()` on all incoming metadata, while `defineLedger()` preserves the exact schema shapes so the generated `index.d.ts` can infer strongly typed metadata fields without boilerplate.
