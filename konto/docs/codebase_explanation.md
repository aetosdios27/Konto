...
- **`konto_holds`**: An ephemeral table managing the two-phase commit (Escrow) system. Tracks uncommitted, earmarked funds that reduce the available balance but haven't hit the immutable journal yet. Built with strict `expires_at` TTL checks to prevent "Zombie Holds" from permanently freezing funds when downstream services crash.
- **`konto_balance_snapshots`**: Foundation for mathematically sound O(1) balance read scaling. Checkpoints balances securely to drastically truncate the derivation scan ranges.
...
- **`getJournals`**: Fetches deeply nested journal layouts, utilizing explicit `LATERAL` joins combined with sub-aggregated `json_agg` boundaries. Includes flawless deterministic keyset pagination scaling efficiently across infinite historical subsets based on strictly sortable UUIDv7 timelines.
